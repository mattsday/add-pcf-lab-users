...
for EMAIL in ${EMAIL_LIST}; do
	...
	echo Apps manager: https://apps.sys.vodafone.apptx.cf/
	echo Ops Manager: https://opsman.vodafone.apptx.cf/
	echo Healthwatch: https://healthwatch.sys.vodafone.apptx.cf/
	echo Metrics: https://metrics.sys.vodafone.apptx.cf/
	echo CLI Commands:
	echo cf login -u "${NAME}" -p "${PW}" -a "${PAS_API}" -o "${NAME}" -s "${NAME}"
	echo pks login -u "${NAME}" -p "${PW}" -a "${PKS_API}" -k
	echo pks get-credentials vodafone
	echo ====================
	{
	uaac target "${PKS_UAA}"
